/*
	...
	# select문의 기본 구조 ('작성 순서') #
        ...
        5. having 그룹 조건: 그룹화 된 대이터에 대한 조건을 지정
...
# - 열 이름을 별칭으로 조회할 때 사용 ( >> 컬럼명 변경x)
# - 공백 사용 시 반드시 따옴표로 묶어야 함
...
#		: is null, is not null 키워드를 통해 null 여부만 확인
...
select * from `members`
where
	name like 'J%'; # 시작만 J이고 뒤는 0개이상의 문자를 허용 / Jin, Joon
...
select * from `members`
where
	name like '_u%'; # 1개의 임의의 문자 + u + 상관없음: 이름의 두번째 글자가 u인 모든 회원 조회

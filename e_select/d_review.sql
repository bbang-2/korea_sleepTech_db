...
select
    ...
    (select sum(amount)
	from purchases
	where member_id = members.member_id) as total_amount
from
	members
where
	member_id = (	
        ...
        from purchases
        group by member_id
        order by sum(amount) desc -- 총 합이 가장 큰 레코드부터 출력
...
# 1) 이름(name)으로 알파벳순 정렬
select * from `members` order by name; -- 오름차순 asc은 생략가능 (desc는 반드시 명시)

# 2) 3번째 레코드부터 2개의 행 조회
select * from `members`
limit 2 offset 2; -- 첫 행의 시작 번호가 0(index 번호와 동일

select * from `members`
limit 2, 2; -- 'offset 시작번호'는 생략 후 콤마 사용 가능

# 3) 포인트(points)가 가장 높은 상위 세명의 회원 조회
select * from `members`
...
select area_code, max(points) as max_point
from `members`
group by area_code;

# 6) 성별(gender)에 따른 평균 포인트 조회
-- 집계함수: avg(), sum(), max(), min(), count()
select gender, avg(points) as avg_points -- as 키워드 생략 가능
from `members`
group by gender;

# 7) 평균 포인트가 250이상인 등급만 조회
select grade, avg(points) as avg_points
from `members`
group by grade
having avg(points) >= 250;

# 8) 등급별 최소 포인트가 100이상인 그룹

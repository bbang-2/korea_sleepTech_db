/*
    ...
    1. 내부 조인(Inner Join): 조건에 일치하는 데이터면 출력(== 교집합)**
		>> 정확히 일치한 행만 조회
        
	2. 외부 조인(Outer Join): 한쪽 테이블에 일치하지 않아도 결과로 포함 (== 합집함)*
		>> NULL 포함 가능
...
/*
	...
	from
		기준 테이블(첫 번째 테이블) as 별칭
			inner join 조인 테이불(두 번째 테이블) as 별칭
			on 조인될 조건
	[where 조건식...];
*/
use `korea_db`;
...
from
	`purchases` as P
        ...
        -- purchases 테이블의 member_id에 존재하는 모든 회원 정보만 출력
where P.member_id = 1;

# cf) 조인 시 두개 이상의 테이블에서 동일한 열 이름이 존재하는 경우
#	, 테이블명(별칭).열이름 형식으로 표기 권장
...
select
	M.name, sum(P.amount) as total_purchases
from
	`members` M
		inner join purchases P -- inner join은 가장 기본 조인이기 때문에 inner키워드 생략 가능
...
select 
	distinct M.name, M.contact -- 조인한 테이블의 데이터가 반드시 조회될 필요는x
from
	`members` M
		join `purchases` P
        on M.member_id = P.member_id;

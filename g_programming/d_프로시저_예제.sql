...
from `member` M
	left outer join `buy` B -- 외부 조인 시 구매 내역이 없더라도 출력o
...
# 2. 스토어드 프로시저
drop procedure if exists CheckPositionㅣ
...
begin
    ...
    select 
        ...
        from `players`
        where
			player_id = p_player_id; # 매개변수 id와 일치하는 선수 id의 데이터 조회
            
		if v_player_position = "타자" then
			select '해당 선수는 타자입니다.';
		else
			select '해당 선수는 타자가아닙니다.';
		end if;
	end $$

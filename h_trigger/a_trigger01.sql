### 트리거 (trigger) ###
# : '방아쇠', 자동 실행
# : DB에서 특정 이벤트(Insert, Update, Delete)가 발생할 때 자동으로 실행되는 SQL코드 블럭

# 트리거 사용 목적
-- 사용자가 실수로 종료한 작업을 빼먹지 않도록 하기 위함 (일정 작업 자동화)
-- 수동으로 처리하는 일련 동작을 자동으로 처리하여 데이터의 오류를 방지 (데이터 무결성)

# ex) 트리거 사용 예시
-- 특정 레코드(행)가 삭제 >> 삭제된 데이터에 대한 로그 기록
-- 데이터 삽입 시 >> 자동으로 관련된 값 추가
-- 회원 정보 수정 시 >> 수정 시간 자동 갱신

# 트리거 동작 방식
-- 이벤트가 발생 할 때만 자동 실행, call문으로 직접 실행이 불가
...
### 트리거(trigger) 실무 사용법 ###
# 특정 테이블에 insert, update, delete가 발생했을 때 '자동 로그'가록
# : 재고 수량 자동 조정, 포인트 자동 적립등 '자동화된 비즈니스 로직 처리'

# cf) 주의점!
# : 남용 시 디버깅이 어렵고 성능 저하 가능성 있음 >>> 꼭! 필요한 곳에서만 사용

### 트리거 기본 문법 ###
# : 스토어드 프로시저와 유사 (cf. call문으로 직접 실행x)
...
create trigger myTrigger
	...
	for each row #각 행마다 적용 시킴(모든 트리거에 작성)
...
-- 2) 수정 테스트
update `trigger_table` set text = '르세라핌' where id = 3;

select @msg;

select * from `trigger_table`;

-- 3) 삭제 테스트`trigger_table`
delete from `trigger_table` where id=4;
select @msg;

### 트리거 vs 트랜잭션 ###
# 1) 트리거
# : 이벤트 발생 시 '자동 실행'되는 SQL코드

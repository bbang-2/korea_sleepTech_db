...
# cf) 데이터 중복
#		: 동일한 정보가 여러 테이블이나 같은 테이블에 여러 행에 중복해서 저장되는 현상
...
create table `student_course_connect` ( -- 학생/과목 관계테이블
	student_id int,
    course_id int,
...
values
	...
	(1, 102),
	(2, 101),
	(3, 103); -- 복합키 설정으로 두개의 컬럼을 합친 내용이 다른 레코드와 중복 x
...
-- 수강 정보 삭제 시 학생과 과목 정보에 영항을 미치지 않음

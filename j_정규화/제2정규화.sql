### 제2정규화 (2NF: Secont Normal From) ###
# : 정규화의 두번째 단계
# - 1NF(제1정규화)를 만족 시키면서 '모든 비기본 속성(일반 컬럼)'이 기본키에 완전히 종속되는 상태
# - 완전 함수적 종속 상태
...
# cf) 완전 함수적 종속
# 		: 속성이 기본키 전체에 종속 되어있으며 기본키 일부분 만으로는 결정 할 수 없는 경우 
#		- 비기본 속성이 '기본키 전체'에 의존하는 것
#		ex) 기본키가 (A, B)일 때, C가 A나 B하나만으로 결정되지 않고, A와 B모두 알아야 C가 정해진다면 완전 종속

# cf) 부분 함수적 종속
#		: 비기본 속성이 기본키의 일부에만 의존하는 것
#		ex) 기본키가 (A, B)일 때, D는 A만으로 결정 된다면 >> 부분 종속 >> 2NF에 위반
...
create table `wrong_2nf` (
    ...
    location varchar(50), -- 부서 위치 (문제발생!, 2NF 위반: department만으로 결정)

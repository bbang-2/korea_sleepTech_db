...
# : 여러개의 작업(명령어)이 한번에 처리되어야 할 때 사용
# - 중간에 하나라도 실패하면 '전체 작업을 취소(Roolback)'
# - 모든 작업이 '문제없이 완료되면 확정(Commit)'

/*
	은행에서 A가 B에게 돈을 보낼 때
    A계좌에서 출금 + B계좌에 입금하는 두 작업은 반드시 함께 처리
    >> 하나만 성공하고 하나는 실패하면 돈이 사라지거나 두번 생기는 문제가 발생
*/

### 2. 트랜잭션의 특징(ACID) ###
# Atomicity(원자성): 모두 성공하거나 모두 실패(All or Nothing)
# Consistency(일관성): 실행 전과 실행 후, 데이터의 일관성이 유지 (DB의 상태가 변경x)
# Isolation(독립성): 동시에 여러개의 트랜잭션이 실행되어도, 서로 간섭하지 않아야함
...
# 3) 성공하면 커밋: 모든 작업이 성공적으로 완료 되면 DB에 변경 사항을 확정(Commit)
...
### 스토어드 프로시저 작성: SQL내의 조건문이나 반복문은 SQL 스크립트 내에서 바로 실행x, 프로시저나 트리거 내에서 사용
delimiter $$
...
select balance into from_balance # select 결과를 변수에 저장 할 때 사용
from accounts
where account_holder = '이승아';

-- 잔액이 1000원 이상일 때만 송금 실행

...
as
	select B.mem_id 'Member id', M.mem_name as 'Member Name' # 별칭(aliase)는 작은/큰 따옴표 가능
...
# cf) 테이블 생성 시 '존재하는 테이블일 경우 생성하지 않는' 옵샨
...
### 뷰 정보 확인 ###
# describe (desc): 열(컬럼)의 정보 확인 / PK, 제약 조건은 확인 불가
...
create view v_hight167
as
	select * from member
where height >= 167;

select * from v_hight167;
# 뷰를 사용한 데이터 삭제 #
delete from v_height167
	where height = 168;
	
    # >> 원본 테이블의 기준(참조 테이블 연결)과 충돌되어 삭제x

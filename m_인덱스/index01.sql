...
# : 기본키(PK) 설정 시 자동 생성
# - 테이블 당 단 하나만 생성 가능
# - 정렬 기능을 포함o: 지정한 열(기본키 설정 열)을 기준으로 자동 정렬
...
# 1) key_name: Primary (기본키 설정으로 자동 생성된 인덱스)
#		>> Primary키 설정으로 인덱스가 자동 부여됨
# 2) Coloumn_name: col1
#		>> col1에 인덱스가 부여되어 있음
# 3) Non_unique: 0
...
select * from members; -- 클러스터형 인덱스가 없는 경우 (PK값이 없는 경우): 데이터 삽입 순서대로 출략
...
alter table members
add constraint unique (mem_id); -- 데이터 삽입 순서대로 출력
